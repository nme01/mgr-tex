...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.35\linewidth]{img/hierarchical-learning_cropped.jpg}
...
$N$) w~celu~wygodniejszego liczenia pochodnej w~procesie uczenia. Zabiego ten ma charakter czysto estetyczny.
...
Wagi warstwy ukrytej aktualizujemy wg wzoru \ref{eqn:wagi_autenc}:
\begin{equation}
\begin{split}
...
\end{equation}
gdzie $f'$ to pochodna funkcji aktywacji używanej w~neuronach, a~$z^{(2)}_i$, to~wartości pre-aktywacji neuronów warstwy
ukrytej (wartości, które trafiają do~funkcji aktywacji tych neuronów).

Wagi wastwy wyjściowej odpowiadają transponowanej macierzy wag warstwy ukrytej. Jest tak~dlatego, że~jak~wcześniej
