...
wartościom wejściowym tej sieci. Inaczej mówiąc: sieć ma za zadanie wykształcenie przekształcenia tożsamościowego
(identycznościowego).
...
\begin{figure}[H]
	...
	\caption{Ograniczona Maszyna Boltzmanna. Na~rysunku: górna warstwa to~warstwa ukryta, a~dolna to warstwa widoczna,
	pełniąca rolę zarówno warstwy wejściowej, jak~i~wyjściowej.}
...
funkcję prawdopodobieństwa p(v), minimalizuje się funkcję średniego ujemnego log-prawdopodobieństwa, gdzie średnia jest
liczona po~wszystkich $T$ przykładach ze~zbioru uczącego:
\begin{equation*}
