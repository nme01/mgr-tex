...
\subsubsection{Lokalna normalizacja odpowiedzi} \label{sssec:normalizacja}
\textbf{Lokalna normalizacja odpowiedzi}\cite{HOG} (\textit{ang.~local response normalization}) polega na~zapewnieniu,
...
\subsubsection{Regularyzacja L2}
...
z~nich są zmniejszane w~liniowy sposób ($W *= (1 -\lambda)$). $\lambda$~to~tzw.~współczynnik regularyzacji, który
im~jest większy, tym~bardziej sieć dąży do~tego, by~mieć równe wartości wszystkich wag (\cite{L2-regularization}).
